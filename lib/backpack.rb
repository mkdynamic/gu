...
class Backpack
  ...
  def connect(use_ssl = false)
    @connection = Net::HTTP.new("#{@username}.backpackit.com", 
      use_ssl ? 443 : 80)
  ...
  def update_item(item_id, content, page_id = current_page_id)
    request "page/#{page_id}/items/update/#{item_id}", "item" => { 
        "content" => content }
  end
  ...
  def move_item(item_id, direction, page_id = current_page_id)
    request "page/#{page_id}/items/move/#{item_id}", 
  ...
  def create_note(title, body, page_id = current_page_id)
    request "page/#{page_id}/notes/create", "note" => { 
        "title" => title, "body" => body }
  end

  def update_note(note_id, title, body, page_id = current_page_id)
    request "page/#{page_id}/notes/update/#{note_id}", "note" => { 
        "title" => title, "body" => body }
  end
  ...
  def create_separator(name, page_id = current_page_id)
    request "page/#{page_id}/separators/create", "separator" => { 
        "name" => name }
  end

  def update_separator(separator_id, name, page_id = current_page_id)
    request "page/#{page_id}/separators/update/#{separator_id}", 
      "separator" => { "name" => name }
  end
  ...
  def share_page(email_addresses, public_page = nil, 
      page_id = current_page_id)
    parameters = { "email_addresses" => email_addresses }
    parameters = parameters.merge({ "page" => { "public" => 
        public_page ? "1" : "0" }}) unless public_page.nil?
  ...
  def create_reminder(content, remind_at = "")
    request "reminders/create", "reminder" => { "content" => content, 
        "remind_at" => remind_at }
  end

  def update_reminder(reminder_id, content, remind_at)
    request "reminders/update/#{reminder_id}", "reminder" => { 
